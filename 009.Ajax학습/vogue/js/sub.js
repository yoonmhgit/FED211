...
///// 각 서브별 데이터 셋팅하기 ///// 체계적으로 데이터의 구조화★★★★★
let sinfo = {
    "fashion": { //객체 데이터안에 객체{}[]가 담겨있음 속성:값 -> 나중에 수정할때 여기서 하나면 바꿔주면됨
        "제목": "Fashion",
        "메뉴": ["전체", "트렌드", "아이템", "피플", "화보"],
        "경로": "fashion", //폴더명
        "타이틀": [
...
$(function () { ///////////////jQB//////////////////////////
    ...
    // 1. 데이터 선택하기
    // 방법 : Get방식으로 받은 파라미터값 pm변수를 이용하여 셋팅된 서브메뉴데이터 객체인 
    //        sinfo속성 중 해당 이름의 속성을 선택하여 값을 셋팅한다. ★★그 해당이름은 pm변수에 담겨있다! pm자리에 ""넣고 안에 데이터확인★★
    let data = sinfo[pm];
    // console.log(data);

    //____________________________________________________________________________________________________________________________
    // 2. 데이터 셋팅하기(데이터를 각페이지안에 셋팅)
    // 원리 : 가져온 데이터에서 속성명으로 각 파트에 맞는 값을 셋팅한다!(다중언어해당)
